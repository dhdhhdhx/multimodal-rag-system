...
public interface MultimodalDocumentRepository extends JpaRepository<MultimodalDocument, Long>, JpaSpecificationExecutor<MultimodalDocument> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT d FROM MultimodalDocument d WHERE d.shared = true AND d.status = 'COMPLETED' AND (LOWER(d.fileName) LIKE LOWER(CONCAT('%',:kw,'%')) OR LOWER(d.extractedContent) LIKE LOWER(CONCAT('%',:kw,'%')))")
